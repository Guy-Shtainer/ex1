...
public class AVLTreeGuy {
	IAVLNode root;

	// constructor for an existing tree
	public AVLTreeGuy(IAVLNode root){
		this.root = root;
	}
	...
	public int insert(int k, String i) {
		...
		if (this.root == null) {
			...
			if (parent.getKey() == 1) // it was unary, no rebalance needed
				...
				if (leftChildRank - rightChildRank == 2){ // either case 2 or 3 where parent is a left child
					if (parent.getLeft().getHeight() - parent.getRight().getHeight() == 1){ // case 2
				...
				} else if (leftChildRank - rightChildRank == -2) { // either case 2 or 3 where parent is a left right
					if (parent.getLeft().getHeight() - parent.getRight().getHeight() == -1){ //case 2
	 ...
	 * postcondition: none
	 */
	public int join(IAVLNode x, AVLTreeGuy t) {
		return -1;
	}
